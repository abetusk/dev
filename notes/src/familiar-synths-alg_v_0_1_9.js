...
function _Z(a,b) {
  ...
  var x = Math.random();
  return Math.floor((b-a)*x) + Math.floor(a);
}

function _z(r) {
  r = ((typeof r === "undefined") ? 1 : r);
  var x = Math.random();
  var y = Math.floor(x*((2*r) + 1)) - Math.floor(r);
  return y;
}
...
function v_0_1_9_rand_instr_poly() {
  var waveform = [ "sine", "saw", "square", "triangle", "pwm" ];
  var filterType = [ "zd24", "diodeZDF", "svf", "biquad", "filter24" ];
  var filterMode = [ "low-pass", "high-pass", "band-pass", "notch" ];
  ...
  opt["detune2_fine"] = true;
  opt["detune3_fine"] = true;

  opt.detune2 += Math.pow(2, _z(0)) - 1;
  opt.detune3 += Math.pow(2, _z(0)) - 1;

  if (opt.detune2_fine) { opt.detune2 += _u()/20.0; }
  if (opt.detune3_fine) { opt.detune3 += _u()/20.0; }

  opt.attack = Math.exp(-_U()*5.0);
  opt.decay   = Math.exp(-_U()*8.0);
  opt.sustain = _U();
  opt.release = _U();

  opt.Q = _U();

  opt.waveform = waveform[_Z(waveform.length)];
  ...
  opt.filterMult = _U();
  opt.cutoff = _min_cutoff + _U(_min_cutoff, _max_cutoff);

  return opt;

}

function v_0_1_9_rand_instr_mono() {
  var waveform = [ "sine", "saw", "square", "triangle", "pwm" ];
  var filterType = [ "zd24", "diodeZDF", "svf", "biquad", "filter24" ];
  var filterMode = [ "low-pass", "high-pass", "band-pass", "notch" ];
  ...
  //opt.detune2 = 1 + Math.pow(2, _z(2));
  //opt.detune3 = 1 + Math.pow(2, _z(2));

  opt.detune2 += Math.pow(2, _z(2)) - 1;
  opt.detune3 += Math.pow(2, _z(2)) - 1;
...
function v_0_1_9(famsynth) {
  var fs = famsynth;

  /*

	delay = Delay({ time:0.125, feedback:0.25});
	verb = Freeverb();
	crush = BitCrusher({sampleRate:0.5, bitDepth:0.5});
	b = Bus2().fx.add(Freeverb());
	b.fx.add(delay);
	b.fx.add(verb);
	b.fx.add(crush);
		
	m = Monosynth({ useADSR: true,
		detune2:-.75, detune3:1, antialias:true, octave:-1,
		decay: 0.15, attack: 0.15, sustain: .0101, release: .15,
		cutoff : 0.9, Q: 0.1,
		filterType : 2, filterMode: 2, filterMult: 1,
		waveform: 'pwm', pulsewidth: 0.5
	})
	.connect(b, 0.5)
	.note.seq( gen( beats(8) * 4 ), Euclid(5,16) )
...
function _ex0() {
  // debug
  var x = v_0_1_9_rand_instr_mono();
  console.log("Clock.bpm = 120;");
  console.log("Theory.mode = \"chromatic\";");
  console.log("Theory.root = \"a4\";");
  console.log('sv = Bus2("spaceverb");');

  console.log("m = Monosynth(" + JSON.stringify(x, null, 2) + ").connect(sv, 0.35);");
  console.log("m.note.seq([ 12 , 7 , 7 , 7 , 7 , 12 , 15 , 7 , 15 , 15 , 7 , 15 , 10 , 7 , 10 , 10 , 10 , 10 , 15 , 8 , 8 , 8 , 15 , 8  ], [  0.125, 0.25, 0.25, 0.125, 0.125, 0.125, 0.125, 0.25, 0.25, 0.125, 0.125, 0.125, 0.125, 0.25, 0.25, 0.125, 0.125, 0.125, 0.125, 0.25, 0.25, 0.125, 0.125, 0.125] );");
  console.log("");
}

function _ex1() {
  // debug
  var x = v_0_1_9_rand_instr_poly();
  console.log("Clock.bpm = 120;");
  console.log("Theory.mode = \"chromatic\";");
  console.log("Theory.root = \"a4\";");
  console.log('sv = Bus2("spaceverb");');

  console.log("m = PolySynth(" + JSON.stringify(x, null, 2) + ").connect(sv, 0.35);");
  console.log("m.note.seq([ 12 , 7 , 7 , 7 , 7 , 12 , 15 , 7 , 15 , 15 , 7 , 15 , 10 , 7 , 10 , 10 , 10 , 10 , 15 , 8 , 8 , 8 , 15 , 8  ], [  0.125, 0.25, 0.25, 0.125, 0.125, 0.125, 0.125, 0.25, 0.25, 0.125, 0.125, 0.125, 0.125, 0.25, 0.25, 0.125, 0.125, 0.125, 0.125, 0.25, 0.25, 0.125, 0.125, 0.125] );");
  console.log("");
