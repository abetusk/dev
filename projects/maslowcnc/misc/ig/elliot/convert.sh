...
# tool radius reduction factor
#
toolF="0.5"
#toolF="1"

toolRi=` echo "$toolR * $toolF" | bc -l`
toolRiC=`echo "$toolRi * $C / $workPreMul" | bc -l`
...
cat $fnOutline.gp $fnInside.gp > combined.gp
...
while [[ "$nline" -gt 0 ]] && [[ "$curiter" -lt "$maxiter" ]] ; do
  ...
  clipcli -F -S evenodd -x $C -s combined.gp > $tmpofn.0
  ...
  curRC=`echo "$curRC  + ($toolRiC * 2)" | bc -l`
done
