...
ctx = {
  ...
  "aruco_width_mm" : 5.0,
  "aruco_spacing_in" : 5.0 * (1.0/25.4),
  "aruco_width_in" : 5* (1.0/25.4),
  "mark_print" : True,
  ...
  "margin_w_in" : (1.0/8.0),
  "margin_h_in" : (1.0/8.0),
  "margin_w_mm" : (1.0/8.0) * 25.4,
  "margin_h_mm" : (1.0/8.0) * 25.4
}
...
print "aruco margin:", ctx["margin_h_in"], ctx["margin_w_in"], "in, ", ctx["margin_h_mm"], ctx["margin_w_mm"], "mm", ctx["margin_h_unit"], ctx["margin_w_unit"], "unit"
...
def make_aruco_image_box(out_fn, ctx, aruco_idx_corners, aruco_idxs):
  ...
  #for y in range(mh, h-mh-chunk, chunk):
  for h_idx in range(0, chunk_steps_h):
    idx[0] = 0

    if idx[1] > max_idx[1]: max_idx[1] = idx[1]

    #for x in range(mw, w-mw-chunk, chunk):
  ...
  if ctx["print_border"]:
    for x in range(mw, w-mw):
      img_array[mh+0][x] = 128
      img_array[mh+1][x] = 128-16
      img_array[mh+2][x] = 128-32
      img_array[h-mh-1][x] = 128
      img_array[h-mh-2][x] = 128-16
      img_array[h-mh-3][x] = 128-32

    for y in range(mh, h-mh):
      img_array[y][mw+0] = 128
      img_array[y][mw+1] = 128-16
      img_array[y][mw+2] = 128-32
      img_array[y][w-mw-1] = 128
      img_array[y][w-mw-2] = 128-16
      img_array[y][w-mw-3] = 128-32
...
make_aruco_image_box("img_11x17/box_5mm_300dpi_11x17_64.jpg", ctx, range(4), range(4, 64+4))
sys.exit(0)
