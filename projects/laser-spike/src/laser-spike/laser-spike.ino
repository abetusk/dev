...
uint16_t g_laser_pwm=0, g_laser_shift_every=16;
uint8_t g_laser_pwm_del = 8;
uint8_t n_pwm = 16, g_laser_shift=0;
...
//g_laser_pwm_del=1;
  ...
  for (pwmnum=0; pwmnum < n_pwm; pwmnum++) {
    //uint16_t t = (g_laser_pwm + (4096/n_pwm)*pwmnum) % 4096 ;
    pwm0.setPWM(pwmnum, 0, (g_laser_pwm + (4096/n_pwm)*pwmnum) % 4096 );
    pwm1.setPWM(pwmnum, 0, (g_laser_pwm + (4096/n_pwm)*pwmnum) % 4096 );

    //Serial.print(pwmnum); Serial.print(" "); Serial.println(t);
  }
...
void laser_diode_geq() {
  ...
  for (i=0; i < n_pwm; i++) {
    ...
    if (y>4095) { y = 4095; }
    if (y<0) { y=0; }
    pwm0.setPWM((i+g_laser_shift)%n_pwm, 0, y);
    pwm1.setPWM((i+g_laser_shift)%n_pwm, 0, y);
  }

  if ((iter_count % g_laser_shift_every)==0) {
    g_laser_shift = (g_laser_shift + 1)%n_pwm;
...
void laser_diode_joystick() {
  ...
  for (pwmnum=0; pwmnum < n_pwm; pwmnum++) {
    ...
    pwm1.setPWM(pwmnum, 0, (uint16_t)f);
  ...
  if (count > 8) {
    if (laser_state == STATE_LASER_GEQ) {
      ...
      */
      ...
      for (int i=0; i<8; i++) {
        int j;
        for (j=0; j<peak[i]; j++) { Serial.print("*"); }
        for (;j<10;j++) { Serial.print(" "); }
        Serial.print(" ");
      }
      Serial.println("");
    }
